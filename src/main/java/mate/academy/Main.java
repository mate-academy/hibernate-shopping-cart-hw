...
public class Main {
    ...
    public static void main(String[] args) throws RegistrationException {
        ...
        User bob = new User();
        bob.setEmail("bob123");
        bob.setPassword("IamBob");
        userService.add(bob);

        AuthenticationService authenticationService = (AuthenticationService) injector
                .getInstance(AuthenticationService.class);
        User ann = authenticationService.register("ann222", "pas22word22");

        ShoppingCartService shoppingCartService = (ShoppingCartService) injector
                .getInstance(ShoppingCartService.class);
        shoppingCartService.registerNewShoppingCart(bob);
        shoppingCartService.registerNewShoppingCart(ann);

        System.out.println(shoppingCartService.getByUser(bob));

        shoppingCartService.addSession(tomorrowMovieSession, ann);
        shoppingCartService.addSession(yesterdayMovieSession, bob);

        System.out.println(shoppingCartService.getByUser(ann));
        System.out.println(shoppingCartService.getByUser(bob));
