...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        User secondUser;
        try {
            user = authenticationService.register("email@mail.ua", "12345");
            secondUser = authenticationService.register("some@mail.com", "12434");
        } catch (RegistrationException e) {
            throw new RuntimeException("Can't register user ", e);
        }
        System.out.println("Adding sessions into shopping cart");
        ShoppingCartService shoppingCartService = (ShoppingCartService)
                injector.getInstance(ShoppingCartService.class);
        System.out.println("Add session for user");
        shoppingCartService.addSession(tomorrowMovieSession, user);
        System.out.println("Add session for secondUser");
