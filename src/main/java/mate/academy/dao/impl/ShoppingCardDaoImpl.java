...
public class ShoppingCardDaoImpl implements ShoppingCartDao {
    ...
    public ShoppingCart add(ShoppingCart shoppingCart) {
        ...
        try {
            ...
            throw new DataProcessingException("Can not add shopping cart: " + shoppingCart, e);
        } finally {
            if (session != null) {
                session.close();
            }
    ...
    public ShoppingCart getByUser(User user) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return session.createQuery("from ShoppingCart s "
                    ...
                    + "left join fetch m.movie left join fetch m.cinemaHall "
                    + "where s.user = :user", ShoppingCart.class)
                    .setParameter("user", user)
                    .getSingleResult();
        } catch (Exception e) {
            throw new DataProcessingException("Can not get shopping cart by user = "
                    + user, e);
        }
    }
