...
public class ShoppingCartDaoImpl implements ShoppingCartDao {
    ...
    public ShoppingCart add(ShoppingCart shoppingCart) {
        ...
        try {
            ...
            throw new DataProcessingException("Can't save shopping cart to DB. "
                    + "Ticket: " + shoppingCart, e);
        } finally {
            if (session != null) {
                session.close();
            }
    ...
    public Optional<ShoppingCart> getByUser(User user) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return Optional.ofNullable(session.createQuery("from ShoppingCart s "
                            + "left join fetch s.tickets "
                            + "where s.user = :user", ShoppingCart.class)
                    .setParameter(USER_PARAMETER, user)
                    .uniqueResult());
        } catch (Exception e) {
