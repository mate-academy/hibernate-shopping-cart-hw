...
public class ShoppingCartDaoImpl implements ShoppingCartDao {
    ...
    public Optional<ShoppingCart> getByUser(User user) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<ShoppingCart> getByUserShoppingCartQuery
                    = session.createQuery("FROM ShoppingCart sc "
                            + "LEFT JOIN FETCH sc.tickets" + " WHERE sc.user = :user",
                    ShoppingCart.class);
            getByUserShoppingCartQuery.setParameter("user", user);
            return Optional.ofNullable(getByUserShoppingCartQuery.uniqueResult());
        } catch (Exception e) {
