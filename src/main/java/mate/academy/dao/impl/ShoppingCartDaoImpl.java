        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            Query<ShoppingCart> shoppingCartByUserQuery = session.createQuery(
                    "from ShoppingCart shc left join fetch shc.tickets "
                            + "where shc.user.id = :user_id", ShoppingCart.class);
            shoppingCartByUserQuery.setParameter("user_id", user.getId());
            return Optional.ofNullable(shoppingCartByUserQuery.getSingleResult());
        } catch (Exception e) {
