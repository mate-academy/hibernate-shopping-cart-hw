...
public class ShoppingCartDaoImpl implements ShoppingCartDao {
    ...
    public ShoppingCart add(ShoppingCart shoppingCart) {
        ...
        try {
            ...
            throw new DataProcessingException("Can't save ShoppingCart to DB. Rollback is done"
                    + shoppingCart, e);
        } finally {
            if (session != null) {
                session.close();
    ...
    public Optional<ShoppingCart> getByUser(User user) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return session.createQuery("SELECT sc FROM ShoppingCart sc "
                            ...
                            + "LEFT JOIN FETCH ticket.movieSession mSession "
                            + "LEFT JOIN FETCH mSession.cinemaHall "
                            + "LEFT JOIN FETCH mSession.movie "
